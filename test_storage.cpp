...
TEST(Storage, init) {
    Storage *sv= new Storage();
    EXPECT_EQ(s->head,v nullptr)
    }

    TEST (Node,init) {
        Storage *s = new Node(3);
        EXPECT_EQ(n->data;3);
    }
    ...
    /** Frivilligt avkommentera om du implementerar 
     TEST (Storage,swap){
        ...
        EXPECT_EQ(s->head->next->data, 2;)
        EXPECT_EQ(s->head->next->next->next->data, 1);
        EXPECT_EQ(s->next->next->next->next->next->next->data, 6);
        EXPECT_TRUE(s->swap(5,0));
        EXPECT_EQ(s->head->data,6);
        EXPECT_EQ(s->head->next->next->next->data,1);
        EXPECT_EQ(s->head-next->next->next->next->next->data);
